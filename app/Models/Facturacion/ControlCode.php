...
class ControlCode extends Model
{
    ...
    public function generarqr($factura) {
        $codeqr = new PrincipalController();
        return $codeqr->crearqr(
            $factura->nitemisor, $factura->nrofactura, $factura->nroautorizacion,
            $factura->fecha, $factura->total, $factura->total, $factura->codigocontrol, $factura->nitcliente, 0, 0, 0, 0,
            $factura->connection
        );
    }
    function generate($authorizationNumber, $invoiceNumber, $nitci,
                      $dateOfTransaction, $transactionAmount, $dosageKey, $connection){        
