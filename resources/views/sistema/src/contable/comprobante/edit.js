...
import { stringToDate, dateToString, convertDmyToYmd, hourToString, convertYmdToDmy } from '../../utils/toolsDate';
...
class EditComprobante extends Component{
        ...
        this.state = {
            ...
            idscuentas: [
                { idcuenta: '', namecuenta: '',
                    ...
                    idcomprobantecuentadetalle: null,
                },
            ],
            idscuentasdel: [],
            totaldebe: 0,
    ...
    get_data() {
        ...
        .then((result) => {
            if (result.response == 1) {
                ...
                var idcomprobantetipo = result.comprobante.fkidcomprobantetipo;
                var numero = result.comprobante.codcomprobante;
                var fecha = convertYmdToDmy(result.comprobante.fecha);
                var nrodoc = result.comprobante.nrodoc == null ? '' : result.comprobante.nrodoc;
                var idmoneda = result.comprobante.fkidmoneda;
                var tipocambio = result.comprobante.tipocambio;
                var idtipopago = result.comprobante.fkidtipopago;
                var contabilizar = result.comprobante.contabilizar == 'S' ? true : false;
                var idbanco = result.comprobante.fkidbanco == null ? '' : result.comprobante.fkidbanco;
                var namebanco = result.comprobante.namebanco == null ? '' : result.comprobante.namebanco;
                var nrocheque = result.comprobante.nrochequetarjeta == null ? '' : result.comprobante.nrochequetarjeta;
                var referidoa = result.comprobante.referidoa == null ? '' : result.comprobante.referidoa;
                var glosa = result.comprobante.glosa == null ? '' : result.comprobante.glosa;

                var idscuentas = [];
                var totaldebe = 0;
                var totalhaber = 0;

                for (let i = 0; i < result.comprobantedetalle.length; i++) {
                    var element = result.comprobantedetalle[i];
                    var cuentafirst = {
                        idcuenta: element.idcuentaplan, namecuenta: element.cuenta,
                        glosas: element.glosamenor == null ? '' : element.glosamenor, 
                        debes: element.debe == 0 ? '' : element.debe, 
                        haberes: element.haber == 0 ? '' : element.haber,
                        namecentrocosto: element.centrodecosto == null ? '' : element.centrodecosto, 
                        idcentrodecosto: element.idcentrodecosto == null ? '' : element.idcentrodecosto,
                        idcomprobantecuentadetalle: element.idcomprobantecuentadetalle,
                    };
                    totaldebe += parseFloat(element.debe);
                    totalhaber += parseFloat(element.haber);
                    idscuentas.push(cuentafirst);
                }
                ...
                this.setState({
                    ...
                    fecha: bandera ? on_data.fecha : fecha,
                    nrodoc: bandera ? on_data.nrodoc : nrodoc,
                    ...
                    contabilizar: bandera ? on_data.contabilizar : contabilizar,
                    idbanco: bandera ? on_data.idbanco : idbanco,
                    namebanco: bandera ? on_data.namebanco : namebanco,
                    nrocheque: bandera ? on_data.nrocheque : nrocheque,
                    referidoa: bandera ? on_data.referidoa : referidoa,
                    glosa: bandera ? on_data.glosa : glosa,
                    idscuentas: bandera ? on_data.idscuentas : idscuentas,
                    totaldebe: bandera ? on_data.totaldebe : totaldebe.toFixed(2),
                    totalhaber: bandera ? on_data.totalhaber : totalhaber.toFixed(2),
    ...
    removeCuenta(index) {
        var idscuentasdel = this.state.idscuentas[index].idcomprobantecuentadetalle;
        if (idscuentasdel != null) {
            this.state.idscuentasdel.push(idscuentasdel);
        }
    ...
    cargarIdsCuentas(idscuentas, glosas, debes, haberes, idscentrocostos, idcomprobantecuentadetalle) {
        for (let i = 0; i < this.state.idscuentas.length; i++) {
            ...
            if (data.idcuenta != '') {
                ...
                idcomprobantecuentadetalle.push(data.idcomprobantecuentadetalle);
            }
        }
    }
    onStoreComprobante() {
        ...
        var haberes = [];
        var idscentrocostos = [];
        var idcomprobantecuentadetalle = [];
        this.cargarIdsCuentas(idscuentas, glosas, debes, haberes, idscentrocostos, idcomprobantecuentadetalle);
        let body = {
            ...
            idcomprobantecuentadetalle: JSON.stringify(idcomprobantecuentadetalle),
        ...
        .then((result) => {
            if (result.response == 1) {
                ...
                message.error('Ocurrio un problema con el servidor.');
                this.setState({
                    loading: false,
                });
            }
...
export default withRouter(EditComprobante);
