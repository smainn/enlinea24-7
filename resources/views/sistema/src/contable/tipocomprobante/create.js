...
class CreateTipoComprobante extends Component{
    ...
    constructor(props){
        ...
        this.storeTipoComprobante = this.storeTipoComprobante.bind(this);
    ...
    onChangeNroActual(value) {
        if (!isNaN(value)) {
            if (value >= this.state.nroinicial) {
                this.setState({
                    nroactual: value
                })
            }
    ...
    storeTipoComprobante() {
        ...
        httpRequest('post', ws.wscomprobantetipo, body)
        .then((result) => {
            //console.log(result);
            if (result.response == 1) {
                message.success('El tipo de comprobante se guardo correctamente');
                this.setState({ redirect: true })
            } else if (result.response == -2) {
                this.setState({ noSesion: true })
            } else if (result.response == 0) {
                message.error('Gestion de Periodo requerido... Favor de registrar Gestion de Periodo');
            }else {
    ...
    showConfirmCancel() {
        ...
        Modal.confirm({
          title: 'Cancelar Nuevo Tipo Comprobante',
    ...
    showConfirmStore() {
        const storeTipoComprobante = this.storeTipoComprobante.bind(this);
        if (!this.validarParametros()) return;
        Modal.confirm({
          title: 'Guardar Tipo Comprobante',
          content: '¿Estas seguro de guardar el tipo de comprobante?',
          okText: 'Si',
          cancelText: 'No',
          onOk() {
            console.log('OK');
    ...
    validarParametros() {
        ...
        let nroini = this.state.nroinicial.trim();
        if (nroini.length === 0) {
            message.warning('El Nro Inicial es requerido');
            return false;
        }
        let nroactual = this.state.nroinicial.trim();
                ...
                <div className="cards">
                    <div className="forms-groups" style={{'marginTop': '-15px'}}>
                        ...
                        <div className="pulls-left">
                            <h1 className="lbls-title"> Crear Tipo Comprobante </h1>
                        </div>
                    </div>
...
export default CreateTipoComprobante;
