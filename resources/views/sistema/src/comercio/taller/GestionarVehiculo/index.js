...
class IndexVehiculo extends Component{
    ...
    getDetalleVehiculo(id) {
        httpRequest('get', ws.wsvehiculoshow + '/' + id)
        .then( result => {
            if (result.response === 1){
                this.setState({
    ...
    anularVehiculo() {
        ...
        httpRequest('post', ws.wsvehiculoanular, vehiculoSeleccionado)
        .then( result => {
            if (result.response === 1){
    ...
    btnEditar(id) {
        if (this.permisions.btn_editar.visible == 'A') {
            return (
                <Link to={routes.vehiculo_edit + '/' + id}
    ...
    onCreateData() {
        var url = routes.vehiculo_create;
