...
export default class IndexFamiliaCiudad extends Component {
    ...
    getFamilia() {
        ...
        httpRequest('get', ws.wsGetCiudades)
        .then((resp) => {
            if (resp.response == -2) {
    ...
    esPadre(id) {
        ...
        for(var i = 0; i < array.length; i++){
            if(array[i].idpadreciudad === id){
                return false;
            }
        }
        for (var i = 0; i < array.length; i++) {
            if (array[i].idciudad === id) {
                if (array[i].idpadreciudad == null) {
                    return true;
                }
            }
                    ...
                    content={
                        <div>
                            <div className="cols-lg-12 cols-md-12 cols-sm-12 cols-xs-12"
                                ...
                                <label className="title-sublogo-content">Categoria: 
                ...
                <Confirmation 
                    ...
                    content={
                        <div className="txts-center" style={{'marginTop': '-10px'}}>
                            <label className="title-sublogo-content"><span>
                                ¿Estas seguro de eliminar {this.state.descripcionPadre} ?</span>
                            </label>
                        </div>
                    }
                />
    ...
    onSubmitCrear(e) {
        ...
        if (this.state.descripcionFamilia.length > 0) {
            ...
            httpRequest('post', '/commerce/admin/familiaciudad', body)
            .then((resp) => {
                if (resp.response === 1) {
                    this.setState({
                        arbolFamilia: resp.data
    ...
    onSubmitEliminar(e) {
        ...
        httpRequest('post', '/commerce/admin/eliminarciudad', {
            idPadre: this.state.idFamiliaElegido
        })
        .then((resp) => {
            if (resp.response === 1) {
            ...
            if (resp.response === 2) {
                this.handleCerrar();
                message.warning('Ups No se puede eliminar por que tiene hijos');
            }
            if (resp.response === 3) {
                this.handleCerrar();
                message.warning('No se puede eliminar por que esta registrado a un Cliente');
            }
        })
        .catch((error) => {
            console.log(error);
                ...
                <div className="cards">
                    <div className="forms-groups">
                        <div className="pulls-left">
                            <h1 className="lbls-title">Gestionar Familia Ciudades</h1>
                        </div>
                        { btnNuevo }
                    </div>
                    <div className="forms-groups">
