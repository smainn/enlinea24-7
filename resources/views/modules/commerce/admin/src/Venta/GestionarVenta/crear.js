...
import { stringToDate, dateToString, dateToStringB, hourToString } from '../../../tools/toolsDate';
...
export default class CrearVenta extends Component{
    ...
    constructor(props){
        ...
        this.state = {
            ...
            saldoApagar: 0,
            NumeroCuota: 1,
            fechaInicioDePago: dateToString(fehcaIniPago),
            tipoPeriodo: 1,
            listaDeCuotas: [],
            alertModal: false,
            messageAlert: '',
    ...
    ventaFecha(date, dateString){
        ...
        var fecha = stringToDate(dateString);
    ...
    getVehiculoDelCliente(idCliente) {
        ...
        httpRequest('get', ws.wsgetvehiculo + idCliente)
        .then((result) => {
            if (result.response == 1 && result.data.length > 0) {
    ...
    validarStock(idalmacen, idproducto, valor, index){
        ...
        then(result => {
            //console.log('RESULT STOCK ', result);
            if (result.response == 1) {
    ...
    validarDatos() {
        ...
        if (this.state.configCodigo && this.state.codigoVenta.length === 0) {
    ...
    modificarFechaPlan(index, fecha, dateString){
        
        var fechaModi = stringToDate(dateString);
        var fechaVenta = stringToDate(this.state.fechaVenta);
    ...
    calculoDePlanPago(){
        ...
        let arr = [];
        let sum = 0;
        let fechaPiv = stringToDate(this.state.fechaInicioDePago);
        var i;
