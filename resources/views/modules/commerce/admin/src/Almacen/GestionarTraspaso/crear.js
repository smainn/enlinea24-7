...
export default class CrearTraspasoProducto extends Component{
    ...
    getTipoTraspaso() {
        httpRequest('get', '/commerce/api/traspaso_producto/get_tipo_traspaso')
        .then((result) => {
            if (result.response == 1) {
                this.setState({
    ...
    getAlmacen() {
        httpRequest('get', '/commerce/api/traspaso_producto/get_almacen')
        .then((result) => {
            if (result.response == 1) {
                this.setState({
    ...
    verificarCodigo(value) {
        if (value.toString().trim().length > 0) {
            httpRequest('get', '/commerce/api/traspaso_producto/validar_codigo/' + value)
            .then((result) => {
                if (result.response == 1) {
                    if (result.valido) {
    ...
    verificarCodigoProducto(value, bandera) {
        var array = [];
                ...
                httpRequest('post', '/commerce/api/traspaso_producto/get_productos_almacenes', body)
                .then((result) => {
                    if (result.response == 1) {
                        this.setState({
    ...
    getStockAlmacenes(idproducto, pos) {
        ...
        httpRequest('post', '/commerce/api/traspaso_producto/get_stock_almacen', body)
        .then((result) => {
            if (result.response == 1) {
    ...
    onSubmit() {
        ...
        httpRequest('post', '/commerce/api/traspaso_producto/store', body)
        .then((result) => {
            if (result.response == 1) {
    ...
    render(){
        ...
        if (this.state.redirect) {
            return (
                <Redirect to="/commerce/admin/traspaso_producto/index"/>
            )
        }
        return (
            <div className="rows">
