...
import { message, Modal, Spin, Icon, TreeSelect, Select } from 'antd';
...
export default class EditVehiculo extends Component{
    ...
    componentDidMount() {
        ...
        httpRequest('get', '/commerce/api/vehiculo/edit/' + this.props.match.params.id + '')
        .then(
            result => {
                if (result.response == -2) {
                    ...
                    // para el componente de imagenes
                    data = result.foto;
    ...
    showCancelConfirm() {
        ...
        Modal.confirm({
            title: '¿Esta seguro de cancelar la actualizacion del vehiculo?',
    ...
    onSubmitGuardarDatos(e) {
        ...
        this.state.arrayDetalle.map(
            ...
            httpRequest('post', '/commerce/api/vehiculo/update', body)
            .then((result) => {
                    if (result.response === 1){
