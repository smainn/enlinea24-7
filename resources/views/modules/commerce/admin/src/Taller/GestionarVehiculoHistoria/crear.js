...
export default class CrearVehiculoHistoria extends Component{
    ...
    componentDidMount() {
        ...
        httpRequest('get', '/commerce/api/vehiculo-historia/create')
        .then(result => {
            if (result.response == -2) {
                ...
                this.setState({
                    ...
                    fechaProximo: this.fechaProxima()
                });
            }
        }).catch(
            error => {
    ...
    getCliente(page, buscar, nroPagina) {
        var url = '/commerce/admin/mostraCliente?page=' + page + '&buscar=' + 
    ...
    showCancelConfirm() {
        ...
        Modal.confirm({
            title: '¿Esta seguro de cancelar el registro de la nueva historia de vehiculo?',
    ...
    confirmarDatos(event) {
        ...
        httpRequest('post', '/commerce/api/vehiculo-historia/post', body)
        .then(result => {      
            this.handleCerrar();     
            if (result.response == 1) {
                this.setState({
